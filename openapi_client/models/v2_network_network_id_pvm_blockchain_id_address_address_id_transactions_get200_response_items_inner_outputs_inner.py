...
class V2NetworkNetworkIdPvmBlockchainIdAddressAddressIdTransactionsGet200ResponseItemsInnerOutputsInner(BaseModel):
    """
    ...
    __properties: ClassVar[List[str]] = ["id", "txId", "index", "type", "amount", "owner", "locktime", "dstChain", "asset"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('base', 'stake', 'reward'):
            raise ValueError("must be one of enum values ('base', 'stake', 'reward')")
        return value
